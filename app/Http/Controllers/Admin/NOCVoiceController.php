...
class NOCVoiceController extends Controller
{
    /**
    ...
    {
        $nocvoice = VoiceRoute::all();
       
        return view('admin.nocvoice.index',compact('nocvoice'));
    }

    public function VoiceAjax(Request $request)
    {
        // dd($request->start);
        ...
        $records = $this->Model->fetchVoice($request, $this->columns);
        $total = $records->get();
        ...
        foreach ($categories as $value) {
            ...
            $action .= '<a href="' . route('admin.nocvoice.show', $value->id) . '" class="toolTip" data-toggle="tooltip" data-placement="bottom" title="View Detail"><i class="fa fa-eye"></i></a>';
    ...
    {
        $nocvoice=VoiceRoute::find($id);
      
        return view('admin.nocvoice.view',compact('nocvoice'));
    }
